...
module.exports = {
    ...
    doResultEntry: (resultDatas) => {
        return new Promise(async (resolve, reject) => {
                    ...
                    if (resultData[0].result[i].ccode == 0) {
                        ...
                        if (ccode) {
                            ...
                            if (candidate) {
                                ...
                                db.get().collection(collection.RESULT_COLLECTION).insertOne(resultData[0].result[i]).then((data) => {
                                    resultData[0].result[i].status = true
                                    resolve(resultData[0].result[i])
                                })
            ...
            // let result = await db.get().collection(collection.RESULT_COLLECTION).findOne({ CCode: resultData.CCode })
            // console.log(result);
            // if (result) {
            //     resolve({ status: false })
            // } else {
            //     db.get().collection(collection.RESULT_COLLECTION).insertOne(resultData).then((data) => {
